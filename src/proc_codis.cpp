...
/* check whether the key has tag or not ,then return it or null */
...
/* compute the key slot number and check whether it has tag, if the key has tag ,compute the slot number according the tag */
...
int proc_slotshashkey(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	for(int i=1; i<req.size(); i++){
		int ret = slots_num(req[i].String().c_str(), NULL, NULL);
		resp->add(ret);
	}
	return 0;
}
